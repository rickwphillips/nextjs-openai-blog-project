    ...
    <div
      ...
      <div className="flex flex-col text-white overflow-hidden">
        ...
        <div className="flex-1 overflow-auto bg-gradient-to-b
          ...
          { posts.map(p => (
            <Link className={`border border-white/0  block text-ellipsis overflow-hidden whitespace-nowrap
              my-1 px-2 bg-white/10 cursor-pointer rounded-sm
              ${postId === p._id ? "bg-white/20 border-white/100" : ""}`}
              key={p._id} href={`/post/${p._id}`}>{p.topic}</Link>
          ))}
        </div>
